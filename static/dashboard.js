...
class LinkedList {
  ...
  insertSorted(data) {
    ...
    const newTs = Date.parse(newNode.data.created_at);
    if (!this.head || newTs > Date.parse(this.head.data.created_at)) {
      newNode.next = this.head;
      this.head = newNode;
    } else {
      let current = this.head;
      while (current.next && Date.parse(current.next.data.created_at) >= newTs) {
...
function getTestSvg(testName) {
  switch (testName) {
    case "Reaction Test":
      return `<svg class="sm:h-24 sm:w-24 sm:block hidden" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="white">
                <g>
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    data.history.forEach(item => {
      ll.insertSorted(item);
    });
    let current = ll.head;
    while (current) {
      const item = current.data;
      const row = document.createElement('div');
      row.className = 'py-2 border-b border-gray-700 flex items-center justify-between';
      const route = testLinks[item.test_name] || '#';
      const svg = getTestSvg(item.test_name);
      row.innerHTML = `
        <a href="${route}" class="flex items-center gap-4 hover:underline test-icon">
          ${svg}
          <span class="text-lg font-semibold">${item.test_name}</span>
        </a>
        <div class="flex flex-col items-end">
          <span class="text-lg font-semibold">Score: ${item.score}</span>
    ...
    if (container.lastChild) {
      container.lastChild.classList.remove('border-b');
    }
  } catch (err) {
    console.error(err);
  }
