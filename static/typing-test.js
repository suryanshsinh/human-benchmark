...
function gameOver() {
    ...
    if (!document.querySelector('body>.buttons')) {
        document.querySelector('body').innerHTML += `
            <div class="flex gap-3 buttons justify-center">
                <button class="py-2 px-5 rounded text-center text-[#17B8BD] text-lg font bg-[#fff1] font-semibold hover:bg-[#fff2] transition-all hover:shadow-lg shadow-md" onclick="saveTest('typing', ${parseFloat(((getWpm() / totalGameTime) * 60).toFixed(2))})">Save score</button>
                <button class="py-2 px-5 rounded text-center text-[#17B8BD] text-lg font bg-[#fff1] font-semibold hover:bg-[#fff2] transition-all hover:shadow-lg shadow-md" onclick="gameRestart()">Try again</button>
            </div>`
    }
	clearInterval(gameTimer)
	...
	document.getElementById('word-counter').innerHTML = `wpm:${parseFloat(((getWpm() / totalGameTime) * 60).toFixed(2))}`
